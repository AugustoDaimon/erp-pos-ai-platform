...
class ProdutoService:
    ...
    def _validar_relacionamentos(self, categoria_id: int | None, subcategoria_id: int | None, marca_id: int | None):
        ...
        # Ajustado para usar find_by_id (o novo padrão das nossas interfaces)
        if categoria_id and not self.categoria_repo.find_by_id(categoria_id):
            raise RelacionamentoNotFoundError(f"Categoria com ID {categoria_id} não encontrada.")
            
        if marca_id and not self.marca_repo.find_by_id(marca_id):
            raise RelacionamentoNotFoundError(f"Marca com ID {marca_id} não encontrada.")

        if subcategoria_id:
            sub = self.subcategoria_repo.find_by_id(subcategoria_id)
            if not sub:
    ...
    def create_produto(self, dto: CreateProdutoDTO) -> ProdutoResponseDTO:
        ...
        # Validação de SKU Único (Ajustado para find_by_sku)
        if dto.sku:
            sku_limpo = str(dto.sku).strip()
            if self.repo.find_by_sku(sku_limpo):
    ...
    def get_produto(self, produto_id: int) -> ProdutoResponseDTO:
        # Ajustado para usar find_by_id
        produto = self.repo.find_by_id(produto_id)
        if not produto:
    ...
    def list_produtos(self, filtro: FiltroProdutoDTO = None) -> list[ProdutoResponseDTO]:
        ...
        if hasattr(filtro, 'estoque_baixo') and filtro.estoque_baixo:
            lista = [p for p in lista if p.estoque_atual <= p.estoque_minimo]
            
        return [ProdutoResponseDTO.from_entity(p) for p in lista]

    def update_produto(self, produto_id: int, dto: UpdateProdutoDTO) -> ProdutoResponseDTO:
        # Ajustado para usar find_by_id
        produto = self.repo.find_by_id(produto_id)
        if not produto:
        ...
        # Validação de SKU (Ajustado para find_by_sku)
        if 'sku' in dados and dados['sku'] is not None:
            novo_sku = str(dados['sku']).strip()
            existente = self.repo.find_by_sku(novo_sku)
            # Verifica se o SKU pertence a outro produto
        ...
        if not atualizado:
             raise RuntimeError("Falha ao atualizar o produto no banco de dados.")
             
        return ProdutoResponseDTO.from_entity(atualizado)

    def delete_produto(self, produto_id: int) -> bool:
        # Ajustado para usar find_by_id
        if not self.repo.find_by_id(produto_id):
            raise ProdutoNotFoundError("Produto não encontrado.")
        
        # O nosso novo repositório retorna um boolean
        sucesso = self.repo.delete(produto_id)
        if not sucesso:
             raise RuntimeError("Falha ao deletar o produto.")
