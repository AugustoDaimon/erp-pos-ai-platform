...
// ATUALIZADO: Suporte para produtos E serviços
export interface CartItem {
  id: string; 
  productId?: number;
  serviceId?: number; // Novo
  name: string;
  qty: number;
  unitPrice: number;
  agendamento?: {     // Novo
  ...
  return (
    <div className="flex-1 bg-[#f8f9fa] flex flex-col overflow-hidden shrink-0 font-sans border border-gray-200">
      ...
      <div className="flex w-full px-4 py-3 border-b border-gray-300 bg-white text-[11px] font-bold text-gray-500 uppercase tracking-wider shrink-0 gap-8">
        <div className="flex-1 pl-1">Produto / Serviço</div>
          ...
          return (
            <div key={item.id} className="flex items-center w-full px-4 py-3 border-b border-gray-100 hover:bg-gray-50 shrink-0 gap-8">
              ...
              <div className="flex-1 flex flex-col min-w-0 pr-4 justify-center">
                <HoverScrollText
                  text={item.name}
                  className={`text-sm font-medium uppercase w-full ${item.serviceId ? 'text-gray-800' : 'text-gray-800'}`}
                />
                {item.agendamento && (
                  <span className="text-[10px] font-black text-gray-600 tracking-wider mt-0.5">
                    {item.agendamento.data.split('-').reverse().join('/')} às {item.agendamento.horario}
                  </span>
                )}
              </div>
